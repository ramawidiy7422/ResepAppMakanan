...
class RecipesFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // launchWhenStarted: "Baca nilai dari 'isNetworkAvailable' ketika kita hanya berada di fragmen Resep."
    ...
    private fun readDatabase() {
        mainViewModel.readRecipes.observeOnce(viewLifecycleOwner) {
                database ->
            // memeriksa bahwa argumen 'backFromBottomSheet' adalah False (dengan nilai True akan membuat permintaan baru ke API)."
        ...
        mainViewModel.recipesLiveData.observe(viewLifecycleOwner) {
            ...
            when(response) {
                ...
                is NetworkStates.Error -> {
                    stopShimmerEffect()
                    // "Kami memuat data dari database ketika tidak ada respons dari layanan API."
                    loadDataFromCache()
